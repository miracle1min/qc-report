...
export const Form2Cabe: React.FC<FormProps> = ({ onBack, showToast, editId }) => {
  ...
  useEffect(() => {
    if (editId) {
      ...
      if (r) {
        ...
        setQty(d.qty || ''); setTemuan(d.temuan || '');
  ...
  const handleSave = () => {
    ...
    const data = JSON.stringify({ shift, kodeProduksi, qty, temuan });
  ...
  return (
    <div className="page-enter">
      ...
      <div style={{ padding: '8px 16px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div><label className="form-label">Qty (gram)</label>
          <input className="cyber-input" type="number" value={qty} onChange={e => setQty(e.target.value)} placeholder="0" /></div>
